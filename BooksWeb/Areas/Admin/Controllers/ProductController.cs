        ...
        public IActionResult Upsert(ProductVM productVM, IFormFile? file)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (file != null)
                {
                    ...
                    // Загружаем файл в наш фолдер по пути 
                    string productPath = Path.Combine(wwwRootPath, @"images\product");

                    // если значение не является пустым или null,это означает, что есть URL-адрес изображения
                    if (!string.IsNullOrEmpty(productVM.Product.ImageUrl))
                    {
                        //удаляем стырй url картинки 
                        var oldImagePath =
                ...
                _unitOfWork.Save();
                TempData["success"] = "Product создан";
